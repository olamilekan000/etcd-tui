...
func (m Model) View() string {
	...
	filterStringStyle := m.textinput.TextStyle.MarginLeft(1).PaddingLeft(1).PaddingRight(0)

	filterPrefixStyle := style.FilterPrefix
	if m.compact {
...
func (m Model) Value() string {
	...
	if val == "type to filter" || val == "'/' to filter" {
		return ""
	}
	return val
}
